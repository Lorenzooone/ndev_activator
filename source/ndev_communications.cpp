...
struct command_ndev_data_prologue {
...
struct command_ndev_data_epilogue {
	uint16_t unk_3;
} PACKED ALIGNED(1);
...
static int ndev_di_bulk_transfer_out(ndev_di_handle* di_handle, const uint8_t* data, size_t length, int* num_transferred) {
	if(!di_handle->connected)
		return -1;
	if(di_handle->handle == NULL)
		return 0;
	return libusb_bulk_transfer_out(di_handle->handle, get_di_ndev_data(), data, length, num_transferred);
}

static int ndev_di_disk_bulk_transfer_out(ndev_di_handle* di_handle, const uint8_t* data, size_t length, int* num_transferred) {
	if(!di_handle->connected)
		return -1;
	if(di_handle->handle == NULL)
		return 0;
	return libusb_disk_bulk_transfer_out(di_handle->handle, get_di_ndev_data(), data, length, num_transferred);
}

static int ndev_di_bulk_transfer_in(ndev_di_handle* di_handle, uint8_t* data, size_t length, int* num_transferred) {
	if(!di_handle->connected)
		return -1;
	if(di_handle->handle == NULL)
		return 0;
	return libusb_bulk_transfer_in(di_handle->handle, get_di_ndev_data(), data, length, num_transferred);
}
...
static int ndev_di_send_command_out(ndev_di_handle* di_handle, const uint16_t* data, uint16_t command, uint16_t num_elems) {
	size_t data_out_size = 0;
	int num_transferred = 0;
	uint8_t* data_out = command_out_generator(data, command, num_elems, &data_out_size);
	int result = ndev_di_bulk_transfer_out(di_handle, data_out, data_out_size, &num_transferred);
...
static int ndev_di_send_command_out_u16(ndev_di_handle* di_handle, uint16_t value, uint16_t command) {
	return ndev_di_send_command_out(di_handle, &value, command, 1);
}

static int internal_ndev_di_send_command_in(ndev_di_handle* di_handle, uint16_t* data, const uint8_t* data_out, size_t data_out_size, uint8_t* data_in, size_t data_in_size) {
	int num_transferred = 0;
	int result = ndev_di_bulk_transfer_out(di_handle, data_out, data_out_size, &num_transferred);
	if(result < 0)
		return result;
	if(num_transferred < data_out_size)
		return -1;
	result = ndev_di_bulk_transfer_in(di_handle, data_in, data_in_size, &num_transferred);
	if(result < 0)
		return result;
...
static int ndev_di_send_command_in(ndev_di_handle* di_handle, uint16_t* data, uint16_t command, uint16_t num_elems) {
	size_t data_out_size = 0;
	...
	int result = internal_ndev_di_send_command_in(di_handle, data, data_out, data_out_size, data_in, data_in_size);
	command_deallocator(data_in);
	command_deallocator(data_out);
	return result;
}

static int ndev_di_send_command_in_u16(ndev_di_handle* di_handle, uint16_t* value, uint16_t command) {
	return ndev_di_send_command_in(di_handle, value, command, 1);
}

ndev_di_handle connect_to_disk_interface(uint8_t* expected_data) {
	ndev_di_handle out_ndev_di_handle;
	out_ndev_di_handle.handle = NULL;
	out_ndev_di_handle.connected = false;
	libusb_connect_to_ndev_di(expected_data, &out_ndev_di_handle);
	return out_ndev_di_handle;
}

static void disconnect_on_failure(ndev_di_handle* di_handle) {
	di_handle->connected = false;
}

static int setup_unk_bytes(ndev_di_handle* di_handle, bool do_first_unk3_send, bool do_final_unk3_send) {
	uint16_t unk_byte2 = 0;
	int result = ndev_di_send_command_in_u16(di_handle, &unk_byte2, NDEV_DI_GET_UNK_BYTE_2);
	if(result < 0)
		return result;
	uint16_t out_unk_byte2 = 0xC3;
	result = ndev_di_send_command_out_u16(di_handle, out_unk_byte2, NDEV_DI_SET_UNK_BYTE_2);
	if(result < 0)
		return result;
	out_unk_byte2 = 0xCF;
	result = ndev_di_send_command_out_u16(di_handle, out_unk_byte2, NDEV_DI_SET_UNK_BYTE_2);
	if(result < 0)
		return result;
	result = ndev_di_send_command_in_u16(di_handle, &unk_byte2, NDEV_DI_GET_UNK_BYTE_2);
	if(result < 0)
		return result;
	out_unk_byte2 = 0xCE;
	result = ndev_di_send_command_out_u16(di_handle, out_unk_byte2, NDEV_DI_SET_UNK_BYTE_2);
	...
	result = ndev_di_send_command_in_u16(di_handle, &unk_byte3, NDEV_DI_GET_UNK_BYTE_3);
	if(result < 0)
		return result;
	uint16_t out_unk_byte3 = 0x85;
	if(do_first_unk3_send) {
		result = ndev_di_send_command_out_u16(di_handle, out_unk_byte3, NDEV_DI_SET_UNK_BYTE_3);
		if(result < 0)
			return result;
		result = ndev_di_send_command_in_u16(di_handle, &unk_byte3, NDEV_DI_GET_UNK_BYTE_3);
		if(result < 0)
	...
	ndev_data_disk_status disk_status;
	disk_status.bits = 0;
	result = ndev_di_send_command_in_u16(di_handle, &disk_status.bits, NDEV_DI_GET_DISK_STATUS);
	if(result < 0)
		return result;
	disk_status.fields.is_request_ready = 0;
	disk_status.fields.is_disk_inserted = 0;
	return ndev_di_send_command_out_u16(di_handle, disk_status.bits, NDEV_DI_SET_DISK_STATUS);
}

void setup_disk_interface_active(ndev_di_handle* di_handle) {
	if(!di_handle->connected)
		return;
	int result = ndev_di_send_command_out_u16(di_handle, 0x000F, NDEV_DI_RESET_USB);
	if(result < 0)
		return disconnect_on_failure(di_handle);
	result = ndev_di_send_command_out_u16(di_handle, 0x000F, NDEV_DI_RESET_USB);
	if(result < 0)
		return disconnect_on_failure(di_handle);
	result = ndev_di_send_command_in_u16(di_handle, &di_handle->hw_rev, NDEV_DI_GET_HARDWARE_REV);
	if(result < 0)
		return disconnect_on_failure(di_handle);
	ndev_data_status status;
	status.bits = 0;
	result = ndev_di_send_command_in_u16(di_handle, &status.bits, NDEV_DI_GET_STATUS_BITFIELD);
	if(result < 0)
		return disconnect_on_failure(di_handle);
	status.bits = 0xFFFF;
	result = ndev_di_send_command_out_u16(di_handle, status.bits, NDEV_DI_SET_STATUS_BITFIELD);
	if(result < 0)
		return disconnect_on_failure(di_handle);
	ndev_data_disk_status disk_status;
	disk_status.bits = 0;
	...
	if(result < 0)
		return disconnect_on_failure(di_handle);
	disk_status.fields.is_request_ready = 0;
	result = ndev_di_send_command_out_u16(di_handle, disk_status.bits, NDEV_DI_SET_DISK_STATUS);
	if(result < 0)
		return disconnect_on_failure(di_handle);
...
void iter_keep_disk_interface_active(ndev_di_handle* di_handle) {
	...
	int result = ndev_di_send_command_in(di_handle, (uint16_t*)&ping_buffer, NDEV_DI_GET_STATUS_PING, sizeof(ping_buffer) / sizeof(uint16_t));
	if(result < 0)
		return disconnect_on_failure(di_handle);
	...
	if(result < 0)
		return disconnect_on_failure(di_handle);
	result = ndev_di_send_command_out_u16(di_handle, (~status.bits) & 0x00C4, NDEV_DI_SET_STATUS_BITFIELD);
	if(result < 0)
		return disconnect_on_failure(di_handle);
	if(!status.fields.disk_request_acknowledged) {
		...
		if(result < 0)
			return disconnect_on_failure(di_handle);
		int num_transferred = 0;
		ndev_di_disk_bulk_transfer_out(di_handle, no_disk_inserted, no_disk_inserted_len, &num_transferred);
		if(result < 0)
			return disconnect_on_failure(di_handle);
...
void disconnect_from_disk_interface(ndev_di_handle* di_handle) {
	if(!di_handle->connected)
		return;
	if(di_handle->handle != NULL)
		libusb_disconnect_from_ndev_di(di_handle);
}
